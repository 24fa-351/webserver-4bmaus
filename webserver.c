...
void send_response(int client_socket, const char *content_type, const char *content, size_t content_length);
...
void send_response(int client_socket, const char *content_type, const char *content, size_t content_length) {
    ...
    snprintf(header, sizeof(header), RESPONSE_HEADER, content_type, content_length);
...
void serve_static(int client_socket, const char *file_path) {
    ...
    snprintf(full_path, sizeof(full_path), "%s/%s", STATIC_DIR, file_path + 7); // remove '/static' part
    ...
    send_response(client_socket, "application/octet-stream", file_content, file_size);
    free(file_content);
}
...
void serve_stats(int client_socket) {
    ...
    snprintf(content, sizeof(content),
             "<html><body><h1>Server Statistics</h1><p>Total requests: %d</p><p>Total received bytes: %zu</p><p>Total sent bytes: %zu</p></body></html>",
...
void serve_calc(int client_socket, const char *query) {
    ...
    char content[BUFFER_SIZE];
    snprintf(content, sizeof(content), "<html><body><h1>Calculation Result</h1><p>%d + %d = %d</p></body></html>", a, b, sum);
